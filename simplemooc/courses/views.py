...
def undo_enrollment(request, slug):
    ...
    if request.method == 'POST':
        enrollment.delete()
        messages.success(request, 'Sua incrição foi cancelada com sucesso')
        return redirect('accounts:dashboard')
...
def announcements(request, slug):
    ...
    context = {
        ...
        'announcements': request.course.announcements.all()
    }
    return render(request, template_name, context)
...
def show_announcement(request, slug, pk):
    announcement = get_object_or_404(request.course.announcements.all(), pk=pk)
